...
This is the canonical upstream-development baseline for AI agents working in this repository. Keep local `AGENTS.md` files short; they point here for the detailed rules.

## Authority order

1. Current project source/tests and explicit project invariants.
...
- Run the plugin QUnit suite through Discourse (`bin/rake "plugin:qunit[discourse-user-cosmetics-store]"`) when frontend tests exist.
...
- Keep backend RSpec coverage for authorization, service/business behavior, retries/idempotency, and failure paths.
...
- Keep authoritative balance, ownership, entitlement, fulfillment, refund, and payment decisions on the server.
...
- Chat integrations: follow the current discourse-chat-integration provider extension model.
