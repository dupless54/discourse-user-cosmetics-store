      ...
      {{else}}
        <section class="cstore-admin__section cstore-admin-wallets">
          ...
          {{#if this.walletResult}}
            <div class="cstore-wallet-card"><header><img src={{this.walletResult.user.avatar_url}} alt="" /><div><strong>{{this.walletResult.user.name}}</strong><span>@{{this.walletResult.user.username}}</span></div><b>{{this.settings.currency_symbol}} {{this.walletResult.wallet.balance}}</b></header>{{#if this.walletResult.wallet.debt}}<div class="cstore-wallet-card__debt"><strong>{{this.walletResult.wallet.debt}} {{this.settings.currency_symbol}} iade borcu</strong><span>Yeni Orb kredileri önce bu borcu kapatır.</span></div>{{/if}}<div class="cstore-wallet-card__stats"><span><strong>{{this.walletResult.wallet.lifetime_earned}}</strong><small>Toplam kazanılan</small></span><span><strong>{{this.walletResult.wallet.lifetime_spent}}</strong><small>Toplam harcanan</small></span>{{#if this.walletResult.wallet.debt}}<span><strong>{{this.walletResult.wallet.debt}}</strong><small>İade borcu</small></span>{{/if}}</div><form {{on "submit" this.adjustWallet}}><label>Miktar<input type="number" value={{this.walletAmount}} {{on "input" this.updateWalletAmount}} placeholder="+100 veya -100" /></label><label>Neden<input maxlength="500" value={{this.walletReason}} {{on "input" this.updateWalletReason}} placeholder="İşlem açıklaması" /></label><button class="btn btn-primary" type="submit">Bakiyeyi güncelle</button></form><section><h3>Son hareketler</h3>{{#each this.walletResult.wallet.ledger as |entry|}}<article><span class={{if entry.credit "is-credit" "is-debit"}}>{{entry.amount}}</span><div><strong>{{entry.reason}}</strong><small>{{entry.entry_type}} · bakiye {{entry.balance_after}}{{#if entry.debt_after}} · borç {{entry.debt_after}}{{/if}}</small></div></article>{{else}}<p>Henüz işlem yok.</p>{{/each}}</section></div>
          {{/if}}
        </section>
      {{/if}}
    </div>
